...
class VPRWrapper:
    ...
    def extract_vlad(self, img_path: str, use_cache: bool = True) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # Target: [N, D] where N is number of patches, D is descriptor dimension
            if desc.dim() == 4:  # [B, C, N, D]
                desc = desc.squeeze(1)  # Remove channel dim -> [B, N, D]
            if desc.dim() == 3:  # [B, N, D]
                desc = desc.squeeze(0)  # Remove batch dim -> [N, D]
            if desc.dim() == 1:  # [D] single descriptor
                desc = desc.unsqueeze(0)  # -> [1, D]
            
            desc = desc.cpu()
            
            # Compute VLAD - expects [N, D] without batch dimension
            vlad_desc = self.vlad.generate(desc)
